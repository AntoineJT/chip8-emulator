...
std::string Chip8::Dump::DumpMemoryWithoutHeap(const Memory& mem)
{
    return Internals::DumpMemoryWithoutHeap(mem).str();
}
...
{
    auto stream = Internals::DumpMemoryWithoutHeap(mem);
    stream << "Memory: " << '\n';
    DumpHeap(stream, mem);
    return stream.str();
}
